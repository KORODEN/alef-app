<template>
  <div class="personInfo">
    <div class="addChild">
      <h2>Дети (макс. 5)</h2>
      <button :disabled="!canAddChild" @click="addChild" v-if="canAddChild">
...
export default {
  ...
  computed: {
    canAddChild() {
      return this.childrenArray.length < 5
    }
  },
  methods: {
    addChild() {
      ...
      this.childrenArray.push(emptyChild)
      this.$emit('modifyChildren', this.childrenArray)
    },
    modifyChild(child) {
    ...
    removeChild(id) {
      this.childrenArray = this.childrenArray.filter(item => item.id !== id)
      this.$emit('modifyChildren', this.childrenArray)
    }
  },
  components: {
